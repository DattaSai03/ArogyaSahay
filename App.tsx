...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (view === 'main' || view === 'onboarding' || (view === 'auth' && user.username)) {
      localStorage.setItem('arogya_active_user', JSON.stringify(user));
    }
  }, [user, view]);
    ...
    const checkMissedDoses = () => {
      ...
      setUser(prev => {
        ...
        if (updated) {
          ...
          const newHistory: HistoryItem[] = missedThisCycle.map(m => ({
          ...
          const newNotifs: Notification[] = missedThisCycle.map(m => ({
            id: Math.random().toString(),
            title: 'Dose Missed!',
